...
function App() {
  ...
  //   admin :"EM-203",
  // }
  function withSuspenseFallback(Component) {
    return function WithSuspenseFallback(props) {
      return (
        <Suspense
          fallback={
    ...
    <div className="flex w-full h-screen">
      ...
      <Routes location={background || location}>
        ...
        {roles?.includes("EM-202") && (
          <Route
            element={withSuspenseFallback(
              <RequireAuth allowedRoles={["EM-202"]} />
            )}
          >
        ...
        {roles?.includes("EM-203") && (
          <Route
            element={withSuspenseFallback(
              <RequireAuth allowedRoles={["EM-203", "EM-201"]} />
            )}
          >
      ...
      {background && (
        ...
        <Routes>
          ...
          <Route
            ...
            path="/new-student"
            element={withSuspenseFallback(() => (
              <RegistrationForm role="EM-201" />
            ))}
          />
          <Route
            exact
            path="/tutor/new-chapter/:unitID"
            element={withSuspenseFallback(ChapterForm)}
          />
          <Route
            exact
            path="/tutor/unit/:unitID/resources/new-resource"
            element={withSuspenseFallback(ResourceForm)}
          />
          <Route
            exact
            path="/tutor/new-lesson/:chapterID"
            element={withSuspenseFallback(LessonForm)}
          />
          <Route
            exact
            path="/admin/courses/new-course"
            element={withSuspenseFallback(CourseForm)}
          />
          <Route
            exact
            path="/course/:courseID/new-unit"
            element={withSuspenseFallback(UnitForm)}
          />
          <Route
            exact
            path="/admin/students/new-student"
