...
const ChapterCard = ({ chapterNumber, chapterName, chapterID, unitID }) => {
  return (
    <article className="hover:cursor-pointer chapter w-full laptop:h-44  h-48 bg-white flex flex-col gap-2  pt-2 rounded-md shadow-lg shadow-slate-300 ">
      <section className="px-2 flex items-center justify-start gap-3 ">
        <div className="ml-2 h-8 aspect-square  rounded-full bg-primary  flex-row-centered text-white ">
          <span className="font-extraLight">{chapterNumber}</span>
        </div>
        <h1 className="uppercase text-lg font-extrabold">{chapterName}</h1>
      </section>
